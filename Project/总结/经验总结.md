...
~~~java
			  ——————————Exception（需要捕捉或需要程序进行处理的异常）——————RuntimeException
			  |													|
父类Throwable——|	  											   ——-其他
              |
			  ————————Error（系统级的错误）
...
- `Error类`：Error表示系统级的错误，一般是由于硬件或运行环境方面的异常，例如内存溢出、虚拟机出错、连接错误等，这类错误不由程序抛出，而且对于这种情况，一般是不用修改程序的，因为程序也无能为力，只能停止运行。 

- `Exception类`：Exception类表示需要捕捉或需要程序进行处理的异常，它的出现原因是程序设计错误或外界输入引起的，是程序必须进行相应处理的。
...
其中关键字**try表示定义一个受到监控、受到保护的程序代码块**；关键字catch与try遥相呼应，定义当try block（受监控的程序块）出现异常时，错误处理的程序模块，并且每个catch block都带一个参数（类似于函数定义时的数那样），这个参数的数据类型用于异常对象的数据类型进行匹配；而throw则是检测到一个异常错误发生后向外抛出一个异常事件，通知对应的catch程序块执行对应的错误处理。
...
#### 1.5.1FileInputStream+FileOutputStream
...
#### 1.5.2BufferedInputStream+BufferedOutputStream(有缓存区更快)
