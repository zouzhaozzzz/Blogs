...
其中关键字**try表示定义一个受到监控、受到保护的程序代码块**；

关键字catch与try遥相呼应，定义当try block（受监控的程序块）出现异常时，错误处理的程序模块，并且每个catch block都带一个参数（类似于函数定义时的数那样），这个参数的数据类型用于异常对象的数据类型进行匹配；
...
#### 1.5.1 FileInputStream+FileOutputStream
...
#### 1.5.2 BufferedInputStream+BufferedOutputStream(有缓存区更快)
